...
mod tests {
    use fracture_engine::{
        BatchScheduler, CacheHandle, PendingRequest, SequenceSlice,
    };
    use fracture_generate::{Sampler, SamplingParams};
    use tokio::sync::mpsc;
    ...
    fn test_prefill_job_to_sequence_slice() {
        ...
        // We need a cache to call schedule(). Use the mock pattern from scheduler tests.
        // Instead, replicate the conversion logic directly.
        // The scheduler loop builds SequenceSlice like this:
        //   SequenceSlice { handle: seq.handle, token_ids: pf.token_ids, positions: pf.positions }
        // So we verify PrefillJob has the right fields by calling schedule with a mock cache.
        // Since we don't have a cache here, test the mapping logic in isolation.
    ...
    fn test_decode_job_to_sequence_slice() {
        // A decode job produces a single-token SequenceSlice.
    ...
    fn test_mixed_batch_slice_construction() {
        // Simulate building slices from a decision with 1 prefill + 1 decode,
        // mirroring the scheduler_loop logic.
    ...
    /// different tokens (probabilistic, but with crafted logits this is reliable).
    #[test]
    fn test_seeded_sampling_different_seeds_different_results() {
        // Create logits where multiple tokens have similar probability.
        // With 1000 roughly-equal logits and temperature=1.0, different seeds
        // should almost certainly pick different tokens.
        let mut logits = vec![0.0f32; 1000];
        for (i, l) in logits.iter_mut().enumerate() {
            *l = (i as f32) * 0.001; // slight gradient so not perfectly uniform
        }
    ...
    fn test_seeded_sampling_deterministic() {
        ...
        assert_eq!(
            token1, token2,
            "same seed should produce the same token"
        );
    }
    ...
    use crate::scheduler_loop::{start_scheduler_loop, SchedulerLoopConfig};
    use fracture_engine::GenerationEvent;
    ...
    struct LoopMockBackend {
        ...
        winning_token: u32,
        vocab_size: usize,
        /// Whether embedding() should return an error (to test forward failures).
        fail_forward: bool,
    }
    ...
    fn make_loop_engine(
        ...
        let cache = PagedKvCacheManager::new(32, cfg.num_layers, cfg.num_kv_heads, cfg.head_dim, engine.backend()).unwrap();
        (engine, cache)
    }
    ...
    async fn test_scheduler_loop_forward_error() {
        ...
        assert!(has_error, "expected GenerationEvent::Error with 'forward pass failed'; got: {events:?}");
    }
    ...
    async fn test_scheduler_loop_cache_alloc_failure() {
        ...
        match result {
            ...
            Ok(Some(other)) => {
                panic!("with 0 cache blocks, expected no Token/Finished events; got: {other:?}");
            }
            Ok(None) => {
    ...
    async fn test_scheduler_loop_stop_token_not_streamed() {
        ...
        let stop_in_tokens = events.iter().any(|e| {
            matches!(e, GenerationEvent::Token(t) if *t == stop_token)
        });
        assert!(
            !stop_in_tokens,
        ...
        let finished = events.iter().any(|e| {
            matches!(e, GenerationEvent::Finished { stop_reason: fracture_core::StopReason::Stop, .. })
        });
    ...
    async fn test_scheduler_loop_concurrent_mock() {
        ...
        for (i, rx) in receivers.into_iter().enumerate() {
            ...
            let finished = events.iter().any(|e| matches!(e, GenerationEvent::Finished { .. }));
